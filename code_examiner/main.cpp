...
void signal_handler(int signal) 
{
    ...
    {
        close(server_fd);
        ns_examiner::log::_log_ptr->fatal(INFO, "fd closed");
    }
    ns_examiner::log::_log_ptr->fatal(INFO, std::format("signal {0:d} triggered", signal));
...
int main()
{
    ...
    componentInit();
    ns_examiner::examiner_server server(6000);
